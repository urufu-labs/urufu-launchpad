...
contract NftStackRhForkTest is Test {
    ...
    function test_EthMint_FlywheelReceives10Pct() public {
        ...
        vm.prank(buyer);
        NftMintModule(mintModule).mint{value: 0.01 ether}(
        ...
        uint256 sinksDelta = (buybackSink.balance - bBefore)
            + (nftSink.balance - nBefore)
            + (treasurySink.balance - tBefore);
        assertEq(sinksDelta + splitterAfter, 0.001 ether, "flywheel captured 10% net");
        assertEq(
            NftMintModule(mintModule).launcherBalance(),
            0.009 ether,
            "launcher accrued 90% via pull"
        );
    }
    ...
    function test_UruMint_UruSinkReceives10Pct() public {
        ...
        assertEq(
            NftMintModule(mintModule).launcherBalanceUru(),
            90e18,
            "launcher accrued 90 URU"
        );
    }
    ...
    function _launchDefaultCollection(bool payWithUru)
        internal
        returns (address token, address mintModule)
    {
        ...
        returns (address buybackSink, address nftSink, address treasurySink)
    {
        ...
        (bool ok1, bytes memory b1) = feeSplitter.staticcall(
            abi.encodeWithSignature("uruBuybackSink()")
        );
        (bool ok2, bytes memory b2) = feeSplitter.staticcall(
            abi.encodeWithSignature("nftRevenueSink()")
        );
        (bool ok3, bytes memory b3) = feeSplitter.staticcall(
            abi.encodeWithSignature("treasurySink()")
        );
